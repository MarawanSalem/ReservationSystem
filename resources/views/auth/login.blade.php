...
<div class="min-h-screen flex items-center justify-center bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
    <div class="max-w-md w-full space-y-8">
        <div>
            <h1 class="text-center text-3xl font-extrabold text-pink-600">ReserveIt</h1>
            <h2 class="mt-6 text-center text-3xl font-bold text-gray-900">Sign in to your account</h2>
            <p class="mt-2 text-center text-sm text-gray-600">
                Or
                <a href="{{ route('register') }}" class="font-medium text-pink-600 hover:text-pink-500">create a new account</a>
            </p>
        </div>
        ...
        <form class="mt-8 space-y-6" action="{{ route('login') }}" method="POST">
            ...
            <div class="rounded-md shadow-sm -space-y-px">
                <div>
                    ...
                    <input id="login" name="login" type="text" required class="appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-t-md focus:outline-none focus:ring-pink-500 focus:border-pink-500 focus:z-10 sm:text-sm" placeholder="Email or username" value="{{ old('login') }}">
                </div>
                <div>
            ...
            <div class="flex items-center justify-between">
                ...
                <div class="text-sm">
                    {{-- <a href="{{ route('password.request') }}" class="font-medium text-pink-600 hover:text-pink-500">Forgot your password?</a> --}}
                </div>
            </div>
